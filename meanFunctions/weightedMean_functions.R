...
formatRow <- function(p1,p2,p3,p4,type) {
	return(as.character(eval(parse(text=type))))
}

roundRow <- function(p,plist) {
	if(!is.na(plist)) {
		if(plist==TRUE) {
			p <- round(p,0)
		} else {
...
formatTable <- function(x, type, p1list, p2list, p3list, p4list,
		formatonly=FALSE) {
	p1 <- x$pattern1
	p2 <- x$pattern2
	p3 <- x$pattern3
	...
	p1list <- p1list == "Percentage"
	p2list <- p2list == "Percentage"
	p3list <- p3list == "Percentage"
	p4list <- p4list == "Percentage"
	p1 <- mapply(roundRow, p1, plist = p1list)
	p2 <- mapply(roundRow, p2, plist = p2list)
	p3 <- mapply(roundRow, p3, plist = p3list)
	p4 <- mapply(roundRow, p4, plist = p4list)

	y <- mapply(formatRow,p1,p2,p3,p4,type)
	y1 <- cbind(x,y, stringsAsFactors=FALSE)
	if(formatonly==TRUE) y1 <- y1[,ncol(y1), drop=FALSE]
	return(y1)
}

# Apply all format to all tables
formatTables <- function(x, type, formatonly=FALSE) {
	...
	p1list <- sapply(type,"[[","pattern1")
	p2list <- sapply(type,"[[","pattern2")
	p3list <- sapply(type,"[[","pattern3")
	p4list <- sapply(type,"[[","pattern4")
	mapply(formatTable, x, typelist, p1list, p2list, p3list, p4list,
		MoreArgs=list(formatonly = formatonly), SIMPLIFY=FALSE)
}
